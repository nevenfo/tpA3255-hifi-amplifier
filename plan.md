...
- [ ] E1.3 Placer filtres LC, sorties, alimentation et boucles de retour. *(EN COURS — deux tranches sur trois. **48 des 124 empreintes placées.**)*
  ...
  - **Tranche « alimentation auxiliaire » = PASS.** Les 24 empreintes du buck `LM5010` et de sa descendance sont posées en colonne dans `x` 158..186, `y` 134..194 — bande libre entre la chaîne d'entrée 48 V, qui s'arrête à `y` ≈ 199, et le haut de carte. Elle touche `U8`/`Q302`, d'où sort `PVDD`, et laisse **`x` 100..155 à l'analogique bas niveau de E1.4**, au bord opposé de `U6` comme l'exige l'orientation dans le coffret. **`U1` reste à rotation 0** : `SW` et `BUCK_VIN` sortent du même bord du WSON-10, donc aucune orientation ne met l'entrée en bas et la sortie en haut ; la continuité de la sortie vers `U2` puis `U3` l'emporte, et `PVDD`, rail DC calme, fait le détour. `R39`/`R40`, le diviseur de retour, sont sur le flanc `FB` en bas à droite, à l'opposé du nœud `SW`. **`C81`/`C82` sont retirés de la tranche** : ils découplent `+12V-OA`, le rail des OPA, et suivront `U4`/`U5` en E1.4.
    ...
    - **Coût inscrit à la baseline : trois `lib_footprint_mismatch`, sur `U1`, `U2` et `U3`.** Tout déplacement IPC fait réécrire l'empreinte par KiCad, qui en retire le bloc `(units)` — 24 blocs supprimés, exactement un par empreinte déplacée — et ferme implicitement deux polygones de sérigraphie de SOT-223. **Aucun UUID n'est perdu : 4543 avant comme après**, aucune géométrie de pastille ni aucun net ne change. C'est le pendant exact des `lib_symbol_mismatch` déjà admis à l'ERC, et **il en apparaîtra à chaque circuit intégré déplacé** — `U4`, `U5`, `U7` en E1.4.
  ...
  - **Reste la tranche « supervision et configuration »** : `U7` TPS3802 et son réseau — `R32`/`R26`/`C67` sur `VSENSE`, `R6`/`C83`/`C84` sur `RESET_RC` — plus les quatre résistances de configuration de `U6` : `R301` `FREQ_ADJ`, `R302` `FAULT`, `R303` `CLIP_OTW`, `R304` `OC_ADJ`.
    - **Contrainte découverte en relevant les pastilles de `U6`** : ces quatre broches sortent **toutes** du flanc `x` = 288,71, sur `y` 168,97..177,86 — c'est-à-dire **en plein dans la zone d'interdiction de la barre**. Aucune ne peut recevoir sa résistance en regard. La contrainte portant sur les pastilles et la hauteur et non sur le cuivre sous vernis, les quatre résistances se posent en colonne `x` = 297, hors des deux bandes interdites, au prix de 5 à 10 mm de piste passant sous la barre.
