...
- [ ] D1.1 Associer chaque composant à une empreinte compatible et disponible. *(PARTIAL. Il ne reste que **quatre** composants sans empreinte, tous relevant de D1.4 : `F301`, `J2`, `J3` et `J4`. `RV1` n'en est pas un : il est volontairement hors carte, voir B2.1 et B2.8. Les `PWR_FLAG` n'en requièrent aucune. Assignés depuis : `U6`, `D301`, `Q301`, `Q302`, puis en D1.5 `R305` à `R310`, `C325`, `C326`, `D302`, et enfin `C110`/`C210`.)*
...
- [ ] D1.6 Corriger l'annotation des cinq `PWR_FLAG` qui portent la référence `?` au lieu de `#FLG0x`. Sans effet ERC, mais `kicad-cli` signale déjà « erreurs de numérotation » et KiCad refuse « Update PCB from schematic » sur un schéma non annoté : **bloquant pour la Phase E**.
