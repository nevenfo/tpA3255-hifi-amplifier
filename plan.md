...
- [ ] E1.5 Revoir symétrie, retour des courants, masses, clearances et manufacturabilité. *(EN COURS — tranche « symétrie mesurée » = PASS, en lecture seule.)*
  ...
  - **Question laissée ouverte, à trancher dans une tranche dédiée** : peut-on ramener `L301` et `L302` à un `x` commun — et de même `L303`/`L304` — sans casser la symétrie L/R déjà acquise ni l'encombrement des quatre chaînes de filtrage sur un flanc unique ? C'est le seul des trois écarts dont la correction n'est pas gratuite.
